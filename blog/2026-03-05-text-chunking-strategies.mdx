...
```
[=====Chunk1=====][overlap][=====Chunk2=====][overlap][=====Chunk3=====]
```
...
- 2026 年学术研究表明：对于通用学术文档，512-token 固定切分的准确率（69%）与递归切分相当，优于语义切分（过度碎片化问题）
  :::
...
```
                ┌──────────────────────────── Parent Chunk (~1000 token) ──────────────────────────────┐
                │                                                                                        │
     ┌──────────┤  [Child 1]          [Child 2]          [Child 3]          [Child 4]  ├──────────┐
     │ ~200 tok │  向量检索目标        向量检索目标        向量检索目标        向量检索目标 │ ~200 tok │
     └──────────┘                                                                       └──────────┘
                 ↑ 1. 向量匹配命中 Child                  ↑ 2. 返回给 LLM 的是 Parent
```
...
```
句子 S1 ──→ Embedding e1 ─┐
句子 S2 ──→ Embedding e2  │──── cosine_sim(e1, e2) = 0.92 ✅ 相似，保持在同一 Chunk
句子 S3 ──→ Embedding e3 ─┘
句子 S4 ──→ Embedding e4  ──── cosine_sim(e3, e4) = 0.31 ❌ 语义跳转！→ 切分点
句子 S5 ──→ Embedding e5 ─┐
句子 S6 ──→ Embedding e6  │──── 新的 Chunk 开始
```
...
```
原始段落：
"张三生于1990年的北京，随后进入清华大学计算机系，
 2015年毕业后加入某知名互联网公司担任算法工程师，
 期间发表了多篇关于向量检索的顶会论文。"

↓ LLM 分解为原子命题 ↓

Proposition 1: 张三出生于1990年。
Proposition 2: 张三出生在北京。
Proposition 3: 张三就读于清华大学计算机系。
Proposition 4: 张三于2015年从清华大学毕业。
Proposition 5: 张三毕业后加入了一家知名互联网公司。
Proposition 6: 张三在该公司担任算法工程师。
Proposition 7: 张三发表了多篇关于向量检索的顶会论文。
```
...
```
文档："Apple 发布了新款 MacBook。这款电脑搭载了 M4 芯片。"

传统切分后 Embedding：
- "Apple 发布了新款 MacBook。" → e1（知道 Apple、MacBook）
- "这款电脑搭载了 M4 芯片。" → e2（不知道"这款电脑"是谁）

Late Chunking：
- 先整体 Embedding，"这款电脑"的向量已经通过注意力机制
  "看到"了前文的 MacBook 信息
- 再按位置切分，每个 Chunk 的向量富含全局语义
```
...
+ LLM 生成上下文说明:
"该数据来自 ACME 公司 2024 年第三季度财务报告，
 Q3 指7月至9月，净利润增长相对于 2023 年同期。"
...
| **语义切分**                      | ⭐⭐⭐⭐⭐ | ⭐⭐⭐     | ⭐⭐⭐     | 低（Embedding）      | 中   | 学术论文、叙述型文档 |
| **父子节点检索**                  | ⭐⭐⭐⭐⭐ | ⭐⭐⭐     | ⭐⭐⭐     | 低                   | 低   | 通用 RAG 优化        |
| **Contextual Retrieval**          | ⭐⭐⭐⭐⭐ | ⭐⭐⭐     | ⭐⭐⭐⭐   | 高（LLM费用）        | 中   | 高精度知识库 QA      |
| **命题式切片**                    | ⭐⭐⭐⭐⭐ | ⭐⭐       | ⭐⭐⭐⭐⭐ | 极高（LLM 大量调用） | 极高 | 专业知识库、合规审查 |
| **Late Chunking**                 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐     | ⭐⭐⭐     | 低                   | 中   | 长文档语义连贯场景   |
...
文本分块技术的演进，本质上是对"**如何让机器理解人类书写逻辑**"这一问题的持续探索：
...
1. **没有万能方案**：最优策略取决于文档类型、查询类型、成本预算的组合
2. **递归切分仍是默认最优基线**：512-token 递归切分在大多数场景下是最佳起点
3. **通过父子节点把精度和上下文两者兼得**：这是性价比最高的升级路径
4. **Embedding 模型的影响 ≥ 切分策略的影响**：先把 BGE-M3 换上去，比换切分策略收益更大
5. **VLM 解析是复杂文档的终极答案**：但成本和延迟仍是生产部署的主要障碍

随着 LLM 上下文窗口持续扩大（GPT-4o: 128K, Gemini 2.0: 1M+），未来可能出现"**不需要切分，直接全文检索**"的 RAG 范式——但在此之前，Chunk 策略仍将是 RAG 质量最关键的工程变量之一。

---

## 参考资料

- [Dense X Retrieval: What Retrieval Granularity Should We Use?](https://arxiv.org/abs/2312.06648) — 命题式切片原始论文
- [Chonkie Documentation](https://docs.chonkie.ai/oss/chunkers/recursive-chunker) — 轻量级 Chunking 库
- [benbrandt/text-splitter](https://github.com/benbrandt/text-splitter) — Rust 高性能文本切分
- [Unstructured.io](https://github.com/Unstructured-IO/unstructured) — 结构化文档解析
- [Infinity-Parser-7B](https://huggingface.co/infly/Infinity-Parser-7B) — SOTA VLM 文档解析
- [PaddleOCR PP-StructureV3](https://github.com/PaddlePaddle/PaddleOCR) — 中文文档理解
- [Anthropic Contextual Retrieval](https://www.anthropic.com/news/contextual-retrieval) — 上下文增强检索
- [Jina Late Chunking](https://jina.ai/news/late-chunking-in-long-context-embedding-models/) — 延迟切分技术
- NVIDIA 文档分块基准研究（2024）
- Chroma Research: SemanticChunking 性能分析（2024）
- NAACL 2025 Findings: 语义切分 vs 固定切分的对比研究
